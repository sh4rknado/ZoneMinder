...
/usr/bin/pacstrap ${TARGET_DIR} base reflector

echo ">>>> install-base.sh: Set Mirror Speedup.."
/usr/bin/arch-chroot ${TARGET_DIR} reflector --verbose -l 5 --sort rate --save /etc/pacman.d/mirrorlist

echo ">>>> install-base.sh: Installing Tools packages.."
/usr/bin/arch-chroot ${TARGET_DIR} pacman -Syy

echo ">>>> install-base.sh: Installing Base packages.."
/usr/bin/arch-chroot ${TARGET_DIR} pacman -S --noconfirm base-devel linux linux-firmware

echo ">>>> install-base.sh: Installing Tools packages.."
/usr/bin/arch-chroot ${TARGET_DIR} pacman -S --noconfirm net-tools nano wget curl rsync git unzip unrar
...
/usr/bin/arch-chroot ${TARGET_DIR} pacman -S --noconfirm gptfdisk openssh syslinux dhcpcd netctl ntp
...
cat <<-EOF > "${TARGET_DIR}${CONFIG_SCRIPT}"
  ...
  localectl set-locale fr_BE.UTF-8
  localectl set-keymap be-latin1
  localectl set-x11-keymap be

  echo ">>>> ${CONFIG_SCRIPT_SHORT}: Configuring Timezone.."
  ...
  echo ">>>> ${CONFIG_SCRIPT_SHORT}: Resize Shared Memory.."
  echo "# Resize Shared memory (shm)" >> /etc/fstab
  echo "tmpfs      /dev/shm      tmpfs   defaults,size=4G   0   0" >> /etc/fstab

  echo ">>>> install-aur.sh: Add group VIDEO to HTTP .."
